...
static ssize_t spi_active(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int rc;
	struct fpc1020_data *fpc1020 = dev_get_drvdata(dev);

	if (!strncmp(buf, "active", strlen("active"))) {
...
static DEVICE_ATTR(spi_active, S_IWUSR, NULL, spi_active);
...
*/
	...
	rc = of_property_read_u32(np, "qcom,qup-id", &val);
	if (rc < 0) {
		dev_err(dev, "qcom,qup-id not found\n");
		goto exit;
	}
	fpc1020->qup_id = val;
	dev_dbg(dev, "qcom,qup-id %d\n", fpc1020->qup_id);
