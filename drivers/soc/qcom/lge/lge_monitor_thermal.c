...
{
	int batt_temp, pmic_therm, pa_therm;

	if((lge_get_pmic_therm() - lge_get_pa_therm()) != 0) {
		batt_temp = lge_get_batt_temp() * 388 / 10000;
		pmic_therm = lge_get_pmic_therm() * 265 / 1000000;
		pa_therm = lge_get_pa_therm() * 148 / 1000;

		vs_temp = (batt_temp + pmic_therm + pa_therm + 7) * 10;
	}
